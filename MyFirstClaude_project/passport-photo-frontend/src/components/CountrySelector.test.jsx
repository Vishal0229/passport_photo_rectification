...
describe('CountrySelector', () => {
  it('renders a select with all 8 country options', () => {
    render(<CountrySelector country="US" onChange={() => {}} />)
    const options = screen.getAllByRole('option')
    expect(options).toHaveLength(8)
    const values = options.map((o) => o.value)
    expect(values).toEqual(
      expect.arrayContaining(['US', 'UK', 'India', 'Canada', 'Australia', 'UAE', 'Schengen', 'Custom'])
    )
  })
  ...
  it('does not render the info card for an unrecognized country code', () => {
    ...
    // All 8 options still render inside the <select>
